...
|---|---|---|
| Двусмысленность | В tests_load.md уточнено: p95 ≤ 500 ms относится только к фазе валидации (без LLM), а не к полному запросу; разделены AS IS и TO BE для лимита 100 KB | problem.md; app/review_service.py:19-22 |
| Непроверяемое требование | Указан конкретный timeout = 10s [TO BE] и ожидаемые коды 504/503 вместо формулировки «контролируемая ошибка» | app/review_service.py:19-22 |
| Пропущенный риск или источник | Добавлен сценарий «Утечка секретов»: diff с тестовым token → проверка, что в промпт ушло [REDACTED] | app/review_service.py:20-21; problem.md |
